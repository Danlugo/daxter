...
| **Connections** ⚠ | **Take over** a model + **bind** its data-source connections to a gateway: `ws takeover` · `ws discover-gateways` · `ws gateway-datasources` · `ws bind-gateway` — **dry-run by default**, gated. In **CLI · MCP (`daxter_take_over`/`daxter_bind_to_gateway`) · web (Connections page)**. Gateway/VNet binding only — shareable cloud-connection "Maps to" is UI-only |
