...
2. Download **[`daxter.mcpb`](https://github.com/Danlugo/daxter/releases/latest/download/daxter.mcpb)** — this link always serves the latest, no version to pick — then **double-click it** (or drag it into **Settings → Extensions**), review the details, and **Install**. The `daxter` tools appear right away — *if they don't, fully restart Claude Desktop once to refresh the extension registry.*
