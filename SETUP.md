...
2. Download **[`daxter.mcpb`](https://github.com/Danlugo/daxter/releases/latest/download/daxter.mcpb)** (this link always serves the latest). Then install it from **inside Claude Desktop** — **Settings → Extensions → Advanced settings → Install Extension…** → pick `daxter.mcpb` (or **drag the file onto the Extensions page**) → review → **Install**. *(On macOS, double-clicking the file also opens the installer; on **Windows** use this Settings flow — double-click isn't reliably wired to Claude.)* The `daxter` tools appear right away — if they don't, fully restart Claude Desktop once.
